...
const App = () => {
  ...
  const renderHome = () => (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-zinc-900">
      ...
      {showFilterModal && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/50 backdrop-blur-sm" onClick={() => setShowFilterModal(false)}>
           <div className="w-full bg-white dark:bg-zinc-900 rounded-t-3xl p-6 animate-in slide-in-from-bottom" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-6">
                 <h3 className="font-bold text-lg dark:text-white">Filtrele</h3>
                 <button onClick={() => setShowFilterModal(false)} className="text-zinc-500">Kapat</button>
              </div>
              <div className="space-y-4 mb-6">
                 <div>
                    <label className="text-sm font-bold text-zinc-500 mb-2 block">Fiyat Aralığı</label>
                    <div className="flex gap-4">
                       <input type="number" placeholder="Min" className="flex-1 p-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl outline-none dark:text-white" />
                       <input type="number" placeholder="Max" className="flex-1 p-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl outline-none dark:text-white" />
                    </div>
                 </div>
                 <div>
                    <label className="text-sm font-bold text-zinc-500 mb-2 block">Kategori</label>
                    <div className="flex gap-2 flex-wrap">
                       {['Elektronik', 'Moda', 'Ev', 'Hobi', 'Araç'].map(c => (
                          <button key={c} className="px-4 py-2 bg-zinc-100 dark:bg-zinc-800 rounded-full text-sm font-medium dark:text-white hover:bg-violet-100 dark:hover:bg-violet-900/30 transition-colors">
                             {c}
                          </button>
                       ))}
                    </div>
              ...
              <button onClick={handleApplyFilters} className="w-full py-4 bg-violet-600 text-white font-bold rounded-xl">Sonuçları Göster</button>
           </div>
        </div>
      )}
    </div>
  ...
  const renderSwipe = () => (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-zinc-900">
       <div className="px-6 py-4 flex justify-between items-center">
          <button onClick={() => setView('home')} className="p-2 bg-white dark:bg-zinc-800 rounded-full shadow-sm"><X size={20} className="dark:text-white" /></button>
      ...
      <div className="h-full bg-white dark:bg-zinc-900 flex flex-col">
         ...
         <div className="flex flex-col items-center -mt-2 pb-6">
            <div className="relative mb-3">
               ...
               <button onClick={() => setView('profile-settings')} className="absolute bottom-0 right-0 bg-violet-600 text-white p-2 rounded-full border-2 border-white dark:border-zinc-900">
                  <Edit size={14} />
               </button>
            </div>
