  ...
  // Chat State
    ...
    // ========================================
    // --- GERİ TUŞU YÖNETİMİ (NİHAİ VE KESİN ÇÖZÜM) ---
    // ========================================

    // 1. Geri tuşuna basıldığında çalışacak tüm mantığı bir useCallback içine alıyoruz.
    // İçindeki değişkenlerden biri bile değişirse, React bu fonksiyonun en güncel halini yeniden yaratır.
    const handleHardwareBack = useCallback(() => {
        // A. Ekranda açık olan Pop-up veya Modallar SADECE kapatılır
        if (confirmationModal) { setConfirmationModal(null); return; }
        if (showProductSwapSelection) { setShowProductSwapSelection(false); return; }
        ...
        if (showPhotoOptions) { setShowPhotoOptions(false); return; }
        
        // Ürün detayı açıksa kapat ve bekle
        if (selectedProduct) { setSelectedProduct(null); return; }

        // B. Alt sayfalardaysak bağlı olduğu ana sayfaya dön
        if (view === 'chat') { setView('matches'); return; }
        ...
        if (view === 'settings' || view === 'blocked-users') { setView('profile'); return; }

        // C. Ana Sekmeler Yönetimi
        const mainTabs = ['home', 'matches', 'upload', 'swipe', 'profile'];
        if (mainTabs.includes(view)) {
            // Ana sayfada (Home) değilsek önce Home'a dön
            if (view !== 'home') {
                setView('home');
                return;
            }
            // Zaten Home sayfasındaysak ve ekranda hiçbir şey açık değilse çıkış yap!
            CapacitorApp.exitApp();
        }
    }, [
        // Bu dizideki her bir değer değiştiğinde fonksiyon güncellenir. Bu sayede ASLA eski veriye bakmaz.
        confirmationModal, showProductSwapSelection, showLocationModal, 
        showFilterModal, showChatMenu, showPhotoOptions, selectedProduct, 
        view, tempReturnProduct, selectedUser
    ]);

    // 2. Bu taze fonksiyonu güvenle tutacak bir Ref oluşturuyoruz
    const backActionRef = useRef(handleHardwareBack);

    // 3. handleHardwareBack her tazelendiğinde, Ref'i güncelliyoruz
    useEffect(() => {
        backActionRef.current = handleHardwareBack;
    }, [handleHardwareBack]);

    // 4. Capacitor dinleyicisini SADECE BİR KERE başlatıyoruz.
    useEffect(() => {
        const listener = CapacitorApp.addListener('backButton', () => {
            // Tuşa her basıldığında Ref içindeki en güncel fonksiyon çalışır!
      ...
      <div className="min-h-screen bg-white dark:bg-zinc-900 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            <h1 className="text-4xl font-black text-[#ffab00] mb-2">TakasLa</h1>
          ...
          <form onSubmit={authMode === 'login' ? handleLogin : authMode === 'signup' ? handleSignup : handleForgotPassword} className="space-y-4">
            ...
            <button 
              ...
              type="submit" 
              className="w-full bg-[#00592e] text-white py-4 rounded-xl font-bold text-lg shadow-lg shadow-violet-200 dark:shadow-violet-900/20 active:scale-95 transition-transform disabled:opacity-50"
            >
          ...
          <div className="mt-6 text-center space-y-2">
            {authMode === 'login' && (
                <>
                  <button onClick={() => setAuthMode('signup')} className="text-zinc-500 font-bold text-sm block w-full">Hesabın yok mu? <span className="text-[#ffab00]">Kayıt Ol</span></button>
                  <button onClick={() => setAuthMode('forgot')} className="text-zinc-400 text-xs font-semibold">Şifremi Unuttum</button>
                </>
            )}
            {authMode === 'signup' && (
                <button onClick={() => setAuthMode('login')} className="text-zinc-500 font-bold text-sm">Zaten hesabın var mı? <span className="text-[#ffab00]">Giriş Yap</span></button>
            )}
            {authMode === 'forgot' && (
              ...
              <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button 
                      ...
                      className="text-xs flex items-center gap-1 text-[#ffab00] font-bold disabled:opacity-50"
                    >
            ...
            <div>
                ...
                <div className="flex flex-col items-center mb-6">
                  <div className="relative">
                      ...
                      <button onClick={() => profileFileInputRef.current?.click()} className="absolute bottom-0 right-0 bg-[#00592e] text-white p-2 rounded-full"><Camera size={16} /></button>
